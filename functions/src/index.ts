  ...
  async (event) => {
      ...
      if (chatData) {
        const recipientIds =
          chatData.participantIds?.filter(
            (id: string) => id !== newMessage.senderId,
          ) || [];
        ...
        const senderName =
          newMessage.senderType === "ai"
            ? "Kochanet AI"
            : senderData?.displayName || "A teammate";
        ...
        for (const recipientId of recipientIds) {
          ...
          try {
            const statusSnap = await rtdb
              .ref(`status/${recipientId}`)
              .once("value");
            const presence = statusSnap.val();
            recipientLooksOffline = presence?.state !== "online";
          } catch (rtdbErr) {
            console.warn(
              `[Push] RTDB presence read failed for ${recipientId}, using Firestore isOnline`,
              rtdbErr,
            );
          ...
          if (recipientData?.pushToken && recipientLooksOffline) {
            const title = chatData.isGroup
              ? `New in ${chatData.name}`
              : senderName;
            const body = chatData.isGroup
              ? `${senderName} @ ${chatData.name}: ${newMessage.text}`
              : newMessage.text;
            ...
            console.log(
              `[Push] Notification sent to ${recipientId} for chat ${chatId} (Badge: ${newBadgeCount})`,
            );
          }
        }
        ...
        if (Object.keys(unreadUpdates).length > 0) {
          ...
          console.log(
            `[Unread] Incremented counts for recipients of chat ${chatId}`,
          );
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if (rawHistory.length >= 20) {
        console.log(
          `[AI] History threshold reached (${rawHistory.length}). Summarizing oldest 10 messages...`,
        );
      ...
      if (lastRequest && now.seconds - lastRequest.seconds < 15) {
        console.log(
          `[AI] Rate limit hit for user ${newMessage.senderId}. Wait 15s.`,
        );
 ...
 */
export const syncTotalUnreadCount = onDocumentUpdated(
  "chats/{chatId}",
  async (event) => {
    ...
    for (const userId of participantIds) {
      if (afterCounts[userId] !== beforeCounts[userId]) {
        ...
        try {
          ...
          console.log(
            `[Badging] Synced totalUnreadCount for user ${userId}: ${total}`,
          );
        } catch (error) {
          console.error(
            `[Badging] Error syncing totalUnreadCount for ${userId}:`,
            error,
          );
        }
      }
